<script lang="ts">
  // Buzz! round: all zones live; first pointerdown wins; type the answer; wrong = lockout.
  ...
  // `$derived` only re-notifies when its VALUE differs by reference — so deriveds over
  // engine internals would freeze after the first render. Snapshot into $state on every
  // engine event (store.rev); $state assignments always notify (safe-equal), keeping the
  // board live across buzzes, guesses and skips.
  const listenLoops = $derived(store.engine?.game.settings.listenLoops ?? 2);
  ...
  const hintText = $derived.by(() => {
    const t = track;
    const lvl = hintLevel;
    if (!t || lvl <= 0) return null;
    const parts: string[] = [];
    if (lvl >= 1 && t.year) parts.push(`Decade: ${Math.floor(t.year / 10) * 10}s`);
    if (lvl >= 2 && t.artist) parts.push(`Artist starts with “${t.artist.trim().slice(0, 1).toUpperCase()}”`);
    if (lvl >= 3 && t.title) parts.push(`Title starts with “${t.title.trim().slice(0, 1).toUpperCase()}”`);
    return parts.join(' · ');
  });
  ...
  async function playClip(): Promise<void> {
    ...
    audio.onended = () => {
      if (unmounted) return;
      const r = store.engine?.currentRound;
      if (!r || r.phase !== 'answer') return;
      if (r.buzzWinnerId) return; // paused during answer
      store.engine?.notifyLoop();
      if (store.engine?.currentRound?.phase === 'answer' && !r.buzzWinnerId) {
        void audio.playTrack(t, { demo: store.demo }).catch(() => {
          failCount += 1;
          tapToPlay = true;
          previewError = 'Preview failed — retry or skip.';
        });
      }
    };
    audio.onerror = () => {
  ...
  // focus the answer field as soon as the buzzer wins the floor;
  // timer is keyed on winner id + round so a late input bind cannot skip it.
  let lastWinnerKey = '';
  $effect(() => {
    void store.rev;
    const key = winnerPS ? `${round?.seq ?? 0}:${winnerPS.player.id}` : '';
    if (answerInput) {
      if (winnerPS) answerInput.focus();
    }
  ...
  function onZoneDown(e: PointerEvent, ps: PlayerState): void {
    if (!round || round.phase !== 'answer') return;
    if (round.buzzLockedSeats.includes(ps.player.seat)) return;
    if (round.buzzWinnerId) return;
    const ok = store.engine?.buzz(ps.player.seat, ps.player.id, e.timeStamp) ?? false;
    if (ok) {
      takeSfx('buzz');
  ...
  function giveUp(auto = false): void {
    ...
    else setTimeout(() => { if (store.engine?.currentRound?.phase === 'answer') audio.resume(); }, 700);
  }
  ...
  async function submit(): Promise<void> {
    ...
    if (!ok) {
      ...
      setTimeout(() => {
        if (store.engine?.currentRound?.phase === 'answer') void audio.resume();
      }, 700);
    }
    answer = '';
  }
...
<div class="board">
  <div class="topline">
    <span class="cat">🔔 Buzz!</span>
    {#if listenLoops > 0}
      <span class="loops tnum">loops {Math.min(listenLoops, (round?.listenLoopsUsed ?? 0) + 1)}/{listenLoops}</span>
    {/if}
    <HoldSkip label="⏭ Skip" />
  </div>
  ...
  {:else if tapToPlay}
    <div class="tap-card error-card">
      ...
      <div class="err-actions">
        <button class="pill pill--cta" onclick={playClip}>↻ Retry</button>
        <button class="pill pill--ghost" onclick={() => { void skipCurrent(); takeSfx('whoosh'); }}>⏭ Skip</button>
      </div>
    </div>
  {/if}
  ...
  {#if winnerPS}
    <div class="winner-overlay" role="dialog" aria-label="Buzz winner">
      ...
      <div class="w-title">
        ...
        <span>buzzed first! Name it:</span>
      ...
      <p class="hint">Tip: “title” or “artist - title”</p>
      </div>
    </div>
  {/if}
...
<style>
  ...
  .zones {
    display: grid;
    grid-template-columns: repeat(var(--zcols, 2), 1fr);
    grid-template-rows: repeat(var(--zrows, 1), 1fr);
    align-content: stretch;
    gap: 12px;
    flex: 1;
    min-height: 0;
  }
  .zones.dimmed .zone:not(.won) {
    opacity: 0.55;
  }
  .zone {
    position: relative;
